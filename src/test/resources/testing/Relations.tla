...
RelComposition(r1, r2) == {<<u[1][1],u[2][2]>> : u \in 
    {x \in RelRanRes(RelDomain(r2),r1) \times RelDomRes(RelRange(r1) ,r2):
        x[1][2] = x[2][1]}}

RelParallelProd(r1, r2) == {<<a, b>> \in (RelDomain(r1) \times RelDomain(r2)) 
                                            \times (RelRange(r1) \times RelRange(r2))
                              : <<a[1],b[1]>> \in r1 /\ <<a[2],b[2]>> \in r2 }

RelPrj1(E, F) == {u \in E \times F \times E: u[1] = u[3]}

RelPrj2(E, F) == {u \in E \times F \times F: u[2] = u[3]}
